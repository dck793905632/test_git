...
<HTML>
	...
	<body>
		<!--  -->
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/AdminProductServlet?method=updateProductByPid&pid=${p.pid}" method="post" enctype="multipart/form-data">
			<input type="hidden" name="pid" value="${p.pid}"/>
			<%--<input type="hidden" name="image" value="${pageContext.request.contextPath}/${p.pimage}"/>--%>
						...
						<select name="cid">
							<c:forEach items="${allCats}" var="c">
								<c:if test="${p.cid==c.cid}">
									...
									<c:forEach items="${allCats}" var="c1">
										<c:if test="${p.cid!=c1.cid}">
											<option value="${c1.cid}">${c1.cname}</option>
										</c:if>
									</c:forEach>
								</c:if>
							</c:forEach>
